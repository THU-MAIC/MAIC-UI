"""
Migration script to remove parent_document_id and add user_prompt column.

SQLite does not support DROP COLUMN directly, so this migration
rebuilds the documents table without parent_document_id and with user_prompt.
"""
...
def migrate():
    ...
    with engine.connect() as conn:
        ...
        if 'parent_document_id' not in existing_columns and 'user_prompt' in existing_columns:
            print("✅ No migration needed.")
            return

        print("📦 Rebuilding documents table to drop parent_document_id and add user_prompt...")
        ...
        conn.execute(text(
            ...
            INSERT INTO documents (
                ...
                root_document_id, version_number, version_label, user_prompt
            )
            SELECT
                id, title, original_filename, file_path, file_size, page_count, subject,
                grade_level, description, user_id, is_public, status, pdf_metadata,
                processing_results, error_message, created_at, updated_at,
                root_document_id, version_number, version_label, NULL
            FROM documents_old
            """
        ))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Remove parent_document_id and add user_prompt")
